...
public class Piece {
	...
	*/
	public boolean equals(Object obj) {
		...
		// YOUR CODE HERE
		if (this.body.equals(other.body)) return true;
		if (Arrays.equals(this.body, other.body)) return true;
		
		for (TPoint p: this.body) {
			boolean found = false;
			for (TPoint op: other.body) {
				if (p.equals(op)) {
					found = true;
					break;
				}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while (!next.equals(root)) {
			current.next = next;
			next = current.next.computeNextRotation();	
